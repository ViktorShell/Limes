...
impl Runtime {
    ...
    pub async fn load_function(
        ...
        let function_id = user_modules.insert_function(user_id, module_id).await?;
...
"#
    ...
    pub async fn exec_function(
        ...
        let function = user_modules.get_function(function_id).await?;
        let result = function.lambda.run(args).await?;
        info!(
            r#"
...
mod tests {
    ...
    async fn module_registration_and_removal() {
        let rt = Runtime::runtime_builder().build().await.unwrap();
        let user_id = rt.register_user().await.unwrap();
        ...
        let mid = rt
            .register_module(
                &user_id,
                &bytes,
                "exec_rust_lambda_function".into(),
                "".into(),
                None,
            )
            .await
            .unwrap();
        assert_ne!(mid, 0);
        rt.remove_module(&user_id, &mid).await.unwrap();
    }
    ...
    async fn load_and_exec_function() {
        ...
        let mid_sort = rt
            .register_module(
                ...
                &sorter_bytes,
                "sorter".into(),
                "sort a list of items".into(),
                Some(
                    r#"{
                    "required": ["items"],
                    "properties": {
                        "items": "string",
                        "description": "items in the form of a,b,d,e,f"
                    }
                }"#
                    .into(),
                ),
        ...
        let mid_op = rt
            .register_module(
                ...
                &op_bytes,
                "op_a_b".into(),
                "execute an op between two variable a and b".into(),
                Some(
                    r#"{
                    "required": ["a", "b", "op"],
                    "properties": {
        ...
        let fid_sort = rt.load_function(&user, &mid_sort).await.unwrap();
        let fid_op = rt.load_function(&user, &mid_op).await.unwrap();

        let (r_sort, r_op) = tokio::join!(
            rt.exec_function(&user, &fid_sort, r#"{"items": "f,e,c,d,a,x"}"#),
            rt.exec_function(&user, &fid_op, r#"{"a": 15, "op": "+", "b": 16 }"#)
        );

        assert_eq!(
            r_sort.unwrap().trim_start(),
            r#"{"content": "[a,c,d,e,f,x]"}"#.trim()
        );
        assert_eq!(
            r_op.unwrap().trim_start(),
            r#"{"content": 31 }"#.trim_start()
        );
    }
    ...
    async fn stop_infinite_loop_via_runtime() {
        ...
        let mid = rt
            .register_module(
                &user,
                &bytes,
                "stop_infinite_loop".into(),
                "is just an infinite loop".into(),
                None,
            )
            .await
            .unwrap();

        let fid = rt.load_function(&user, &mid).await.unwrap();
